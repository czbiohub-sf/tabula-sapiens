...
def setup_dataset(input_anndata, ts_filepath, tissue = None, use_10X_only = False, batch_correction_conditions = None):
    ...
    full.obs['_dataset'] = ['tabula_sapiens'] * train_data.n_obs + ['user'] * test_data.n_obs
    return full
...
def scanvi_annotation(
    ...
    scanvi = SCANVI(scvi_data.nb_genes, scvi_data.n_batches, scvi_data.n_labels, n_layers=3, n_latent=50,
                    dispersion='gene-batch') # symmetric_kl=True
    ...
    #should be scvi
    full = trainer_scanvi.create_posterior(trainer_scanvi.model, user_data, indices=np.arange(len(user_data)))
    _, pred = full.sequential().compute_predictions()

    test_data.obs['scanvi_pred'] = pred
...
def svm_annotation( 
    ...
    full_dataset.obs[batch_key] = full_dataset.obs[batch_key].astype('category')
    # feature selection
    sc.pp.highly_variable_genes(full_dataset, min_mean=0.0125, max_mean=3, min_disp=0.5, batch_key=batch_key)
    var_select = full_dataset.var.highly_variable_nbatches > 1
    var_genes = var_select.index[var_select]
    full_dataset = full_dataset[:, var_genes]

    train_X = full_dataset[:train_data.n_obs].X
    test_X = full_dataset[train_data.n_obs:].X
    train_Y = full_dataset[:train_data.n_obs].obs[ts_label_key]
    ...
    train_data.write(svm_output_path)
    
def onclass_annotation(input_anndata, 
                       output_folder, 
                       ts_filepath = 'data/OnClass_data/data_used_for_training/tabula-muris-senis-facs_cell_ontology_test.h5ad',
                       ts_label_key = 'manual_cell_ontology_class'):
    ...
    train_X, train_genes, train_Y = read_data(feature_file=ts_filepath, tp2i = tp2i, AnnData_label=ts_label_key)

    print('Loading the new dataset.')
    test_X, test_genes, test_Y, test_AnnData = read_data(feature_file=input_anndata, tp2i = tp2i, return_AnnData=True, AnnData_label='cell_ontology_id')

    print('Predicting the labels of cells')# in droplet cells. Scanorama is used autoamtically to correct batch effcts.:
...
def singlecellnet_annotation():
    ...
    batch = list(np.unique(data.obs[ts_batch_key]))
    batch_id = [batch.index(x) for x in data.obs[ts_batch_key]]
    ...
    train_X = data[:train_data.n_obs].X.todense()
    train_Y = data[:train_data.n_obs].obs[['tissue', 'batch', ts_label_key]]

    R_train_X = ro.r.matrix(train_X)
    R_train_Y = ro.conversion.py2rpy(train_Y)

    print('Finding genes')
    cgenes2 = singleCellNet.findClassyGenes(R_train_X, R_train_Y, ts_label_key)
